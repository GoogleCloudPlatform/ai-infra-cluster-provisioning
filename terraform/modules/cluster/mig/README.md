...
|------|-------------|------|---------|:--------:|
| <a name="input_container_image"></a> [container\_image](#input\_container\_image) | Container image to start on boot on each instance. When this is set, the default for machine\_image will be changed to `{ project = "cos-cloud", family = "cos-stable" }`. | `string` | `null` | no |
