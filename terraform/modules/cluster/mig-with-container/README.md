...
|------|-------------|------|---------|:--------:|
| <a name="input_container"></a> [container](#input\_container) | Container image to start on boot on each instance. When this is set, the default for machine\_image will be changed to `{ project = "cos-cloud", family = "cos-stable" }`. All `local_mount`s found in `filestore_new` and `gcsfuse_existing` will be visible within the container. | <pre>object({<br>    image = string<br>    cmd   = string<br>  })</pre> | `null` | no |
