...
### Shared Filesystem
For sharing data across machines running AI workload, users need a shared file system. Currently we are using NFS filestore or GCS bucket as shared file systems across machine. Users can use the `GCS_MOUNT_LIST` parameter to provide a comma separated list of GCS buckets and their mount path. Similarly they can use `NFS_FILESHARE_LIST` parameter to provide comma separated list of path. For each filestore path, a new filestore will be created and mounted to the path specified on every VM in the cluster.
